...
#parser$add_argument("-a", dest = "authorization", type = "character", default = "~/.synapseConfig",
#                    help = "Path to .synapseConfig file")
...
if (overview) {
  ...
  for (check_no in check_nos_valid) {
    cat(glue("- check {sprintf('%02d', check_no)} ({config$checks[[check_no]]$label}): {config$checks[[check_no]]$description}"), "\n")
  }
} else {
